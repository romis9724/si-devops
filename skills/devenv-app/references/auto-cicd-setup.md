...
SKILL.md **PHASE 6**의 6-0 ~ 6-4 단계에서 실제로 실행하는 GitLab/Jenkins/Nexus/SonarQube API 호출 절차를 모은 문서입니다. SKILL.md는 의도/제약만 기술하고, 구체 명령은 여기서 lookup합니다.

샘플 앱은 **backend/frontend/admin 3개 독립 repo + 3개 독립 Jenkins job**으로 구성됩니다.

> 역사적 배경: 본 절차는 초기에 `scripts/post-install.sh`라는 단일 셸 스크립트로 작성됐습니다. 현재는 에이전트가 PHASE 6에서 직접 실행하지만, 아래 명령들은 그대로 재사용 가능합니다.

## SKILL.md PHASE 6 매핑

| 본 문서 §  | SKILL.md 단계 | 책임 |
|----------|--------------|------|
| §1, §2   | 6-0 사전 준비 | GitLab root 토큰 발급 / root 계정 폴백 |
| §3       | 6-2 GitLab 저장소 생성 | 그룹 + 3개 프로젝트 생성 |
| §4       | 6-2 GitLab 저장소 push | sample-apps push (NTFS chmod 우회 포함) |
| §5       | 6-0 사전 준비 | Nexus 비밀번호 추출 + Docker hosted repo 생성 |
| §6       | 6-0 사전 준비 | SonarQube 토큰 발급 + Webhook 등록 |
| §7       | 6-3 Jenkins 잡 생성 | JCasC 적용 |
| §8       | 6-3 Webhook 등록 | GitLab → Jenkins |
| §9       | 6-4 첫 빌드 모니터링 | 3개 잡 동시 폴링 |
| §10      | PHASE 7 완료 요약 | 결과 출력 |
...
```
[PHASE 5 완료 — DB + placeholder 컨테이너 가동됨]
        │
        ▼
[PHASE 6 시작]
        │
        ├── 6-0 사전 준비
        │     ├── 1. GitLab root 토큰 발급/검증 (Rails 콘솔, §1~2)
        │     ├── 2. SonarQube 토큰 발급 (§6, devenv-security 감지 시)
        │     └── 3. Nexus docker-hosted repo 생성 (§5)
        │
        ├── 6-1 샘플 앱 코드 생성 (sample_app_level + framework 분기)
        │
        ├── 6-2 GitLab 저장소 생성 + push (§3~4)
        │       - {project_name}/backend
        │       - {project_name}/frontend
        │       - {project_name}/admin
        │
        ├── 6-3 Jenkins JCasC 적용 → 3개 잡 등록 (§7)
        │       + GitLab Webhook 3개 등록 (§8)
        │
        └── 6-4 첫 빌드 모니터링 (§9)
              └── 결과를 preset.json `app.phaseProgress`에 기록
```
...
모니터링 성공 판정 전에 repo별로 아래 게이트를 순차 검증합니다.

1. Build Gate: Jenkins build result = SUCCESS
2. Test Gate: repo 테스트 통과 (`mvn test` 또는 `npm run test`)
3. Security Gate: Sonar Quality Gate PASS + Trivy HIGH/CRITICAL=0 + Dependency-Check PASS
4. Artifact Gate: Nexus에 `${repo}:latest` manifest 조회 성공
5. Deploy Gate: 컨테이너 상태 `Up` + restart loop 없음
6. Smoke Gate: 서비스 endpoint 200 응답

한 게이트라도 실패하면 해당 repo만 FAILURE로 고정하고, 다른 repo 검증은 계속 진행합니다.

### 9-2. LLM 토큰 절약형 출력 포맷

운영 로그를 그대로 출력하지 말고 아래 포맷으로 요약 출력합니다.

```text
[build-monitor]
backend=SUCCESS  frontend=RUNNING  admin=FAILURE
last-failure=admin: Deploy Gate (container restart loop)
next-action=retry-admin-only
```

실패 케이스에서만 `해당 repo`의 마지막 콘솔 30~50줄을 추가합니다.

### 9-3. 실행 로그 정합 규칙 (문서-실행 일치)

`SKILL.md`의 PHASE 6 템플릿과 실제 실행 로그 포맷을 동일하게 유지합니다.

정합 규칙:
1. 필드명 고정: `gate.build`, `gate.test`, `gate.security`, `gate.artifact`, `gate.deploy`, `gate.smoke`, `result`, `next`
2. 상태값 고정: `PASS|FAIL|SKIP|SUCCESS|FAILURE|NOT_RUN`
3. 요약 우선: 성공 시 단일 요약 블록 1회 출력, 반복 폴링 로그는 생략
4. 실패 상세: 실패 repo만 콘솔 tail(30~50줄) 출력
5. 재시도 계획: 실패 시 반드시 `retry-plan=<repo-only|all>` 포함

권장 출력 예시:

```text
[build-monitor]
backend=SUCCESS  frontend=FAILURE  admin=NOT_RUN
gate.frontend=PASS/PASS/PASS/PASS/PASS/FAIL
retry-plan=frontend-only
next-action=wait-user-input(1|2|3)
```

### 9-4. 통합 관점 점검 요약 (운영 리포트)

빌드/배포 종료 후 아래 요약을 1회 출력합니다.

```text
[run-summary]
infra=OK            # endpoint/컨테이너 상태
developer=OK        # 샘플 앱 실행/디버그 정보
devops=OK           # pipeline/deploy/smoke
harness=OK          # gate/partial-retry/standard-format
overall=SUCCESS|PARTIAL|FAIL
```

`overall` 기준:
- `SUCCESS`: repo 전부 4게이트 PASS
- `PARTIAL`: 일부 repo FAIL, 나머지 PASS
- `FAIL`: 핵심 선행조건/공유 자원 실패로 전체 진행 불가

### 9-5. 운영 대시보드 연동용 최소 JSON 스키마

`run-summary` 텍스트와 동일 정보를 JSON으로도 출력할 수 있도록 아래 최소 스키마를 사용합니다.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "devenv-app-run-summary",
  "type": "object",
  "required": [
    "runId",
    "project",
  ...
  "properties": {
    ...
    "phase": { "type": "string", "enum": ["PHASE6", "PHASE7"] },
    "timestamp": { "type": "string", "format": "date-time" },
    "status": { "type": "string", "enum": ["SUCCESS", "PARTIAL", "FAIL"] },
    "views": {
      "type": "object",
      "required": ["infra", "developer", "devops", "harness"],
      "properties": {
        "infra": { "type": "string", "enum": ["OK", "WARN", "FAIL"] },
        "developer": { "type": "string", "enum": ["OK", "WARN", "FAIL"] },
        "devops": { "type": "string", "enum": ["OK", "WARN", "FAIL"] },
        "harness": { "type": "string", "enum": ["OK", "WARN", "FAIL"] }
      },
      "additionalProperties": false
    },
    "repos": {
      "type": "object",
      "required": ["backend", "frontend", "admin"],
      "properties": {
        "backend": { "$ref": "#/$defs/repoStatus" },
        "frontend": { "$ref": "#/$defs/repoStatus" },
        "admin": { "$ref": "#/$defs/repoStatus" }
      },
      "additionalProperties": false
    },
    "retryPlan": { "type": "string", "enum": ["none", "repo-only", "all"] },
  ...
  "$defs": {
    "repoStatus": {
      ...
      "properties": {
        "result": { "type": "string", "enum": ["SUCCESS", "FAILURE", "NOT_RUN"] },
        "gates": {
          "type": "object",
          "required": ["build", "artifact", "deploy", "smoke"],
          "properties": {
            "build": { "type": "string", "enum": ["PASS", "FAIL", "SKIP"] },
            "artifact": { "type": "string", "enum": ["PASS", "FAIL", "SKIP"] },
            "deploy": { "type": "string", "enum": ["PASS", "FAIL", "SKIP"] },
            "smoke": { "type": "string", "enum": ["PASS", "FAIL", "SKIP"] }
          },
          "additionalProperties": false
        },
...
```json
{
  "runId": "run-20260428-193200",
  "project": "myproject",
  "phase": "PHASE6",
  "timestamp": "2026-04-28T19:32:00+09:00",
  "status": "PARTIAL",
  "views": {
    "infra": "OK",
    "developer": "WARN",
  ...
  "repos": {
    "backend": {
      "result": "SUCCESS",
      "gates": { "build": "PASS", "artifact": "PASS", "deploy": "PASS", "smoke": "PASS" }
    },
    "frontend": {
      "result": "FAILURE",
      "gates": { "build": "PASS", "artifact": "PASS", "deploy": "PASS", "smoke": "FAIL" },
      "lastFailure": "Smoke gate: GET / returned 502"
    },
    "admin": {
      "result": "NOT_RUN",
      "gates": { "build": "SKIP", "artifact": "SKIP", "deploy": "SKIP", "smoke": "SKIP" }
    }
  },
  "retryPlan": "repo-only",
  "nextAction": "wait-user-input(1|2|3)"
}
```

대시보드 연동 최소 규칙:
1. 저장 키: `runId + timestamp`
2. 실패 알림 조건: `status != SUCCESS`
3. 자동 재시도 후보: `retryPlan=repo-only` 이고 실패 repo가 1개인 경우
4. 장기 추세 지표: `status`, repo별 gate fail 빈도, 평균 재시도 횟수
5. PII/비밀 정보 금지: 토큰/비밀번호/원문 헤더 저장 금지

...
> **경로**: 본 문서의 `scripts/*.sh`·`install-all.sh` 등은 **`devenv-core`가 `generate-configs.sh`로 생성한 `${DEVENV_HOME}`** (예: `~/devenv-myproj`) 기준입니다. `devenv-app` Git 루트에는 해당 파일이 없습니다.
...
# 현재 스킬은 3-repo 분리 구조이므로 backend/frontend/admin 각각 적용:
for repo in backend frontend admin; do
  sed -i '1s/^\xEF\xBB\xBF//' ~/devenv-<project>/sample-apps/${repo}/Jenkinsfile
done
```
...
# 빌드 트리거 (Crumb 포함, 3-repo 분리이므로 잡명은 {project}-{backend|frontend|admin})
curl -sS -X POST -u "admin:<PASSWORD>" \
  -H "${CRUMB_FIELD}: ${CRUMB_VALUE}" \
  "http://localhost:8080/job/<project>-backend/build"
```
...
**해결**: triggers 블록 제거. GitLab Webhook은 SKILL.md PHASE 6 6-3에서 에이전트가 API로 직접 등록.
...
GITLAB_PW=$(jq -r '.core.gitlabRootPassword' ${DEVENV_HOME}/preset.json)
git remote set-url origin "http://root:${GITLAB_PW}@localhost:<GITLAB_PORT>/<PROJECT>/<repo>.git"
...
**원인**: backend repo 초기 push 시 `gradlew` 파일이 없음.
**해결**: Docker로 wrapper 생성 후 push.
```bash
cd sample-apps/backend
...
GITLAB_TOKEN=$(jq -r '.core.gitlabToken' ${DEVENV_HOME}/preset.json)
git remote set-url origin "http://root:${GITLAB_TOKEN}@localhost:<GITLAB_PORT>/<PROJECT>/backend.git"
git push origin main
```
...
## 11. PHASE 6 부분 실패 대응 연결

PHASE 6에서 일부 repo만 실패한 경우, 임의 문구로 대응하지 말고 `SKILL.md`의 아래 섹션을 그대로 사용하세요.
...
failed-stage=<build|artifact|deploy|smoke>
last-http-code=<status>
last-log-tail=<30~50 lines>
retry-plan=<repo-only|all>
```

이 정보가 있으면 PHASE 6의 1/2/3 선택지로 즉시 재진입할 수 있습니다.

...
이 문서는 상세/심화 대응용입니다.
빠른 1차 대응은 `references/quick-troubleshooting.md`를 먼저 사용하세요.
...
## A. 우선 해결 순서 (Infra/DevOps/Harness)

설치 실패를 가장 크게 줄이는 우선순위입니다. 상세 원인은 각 본문 섹션을 참고하세요.

### A-1. P0 - `--remove-orphans` 제거 (가장 우선)
- 대상: `install-{bastion,gitlab,nexus,jenkins}.sh`
- 조치: `docker compose ... up -d --remove-orphans`에서 `--remove-orphans` 제거
- 이유: 동일 `--project-name` 공유 시 선행 서비스가 orphan으로 삭제됩니다.

### A-2. P1 - Preflight/Bootstrap 자동 교정
- WSL2 Ubuntu-22.04 설치 여부 + OOBE 완료 상태 확인
- `vm.max_map_count=262144` 즉시 반영 + `/etc/sysctl.conf` 영구 반영
- Docker `insecure-registries` 등록:
  - `127.0.0.1:5000`
  - `localhost:5000`
  - `10.0.1.12:5000`

### A-3. P1 - PowerShell↔WSL 경계 표준화
- `wsl.exe` 호출은 PowerShell 경로로만 통일
- 인라인 쿼팅(`bash -c "sed -i ..."`) 금지
- 파일 기반 스크립트(`.patch.sh`)를 저장 후 `wsl.exe ... -- bash /mnt/c/.../.patch.sh` 실행

### A-4. P1 - Readiness 기반 설치 오케스트레이션
- 고정 sleep 또는 부모 PID wait 기반 폴링을 사용하지 않습니다.
- 순차 설치:
  - `bastion -> gitlab -> nexus -> jenkins`
- readiness 기준:
...
### A-5. P2 - Harness 판정 기준 강화
- 성공 기준을 "컨테이너 running"에서 "서비스 readiness + 포트 응답 + 핵심 시나리오"로 강화
- Bastion 내부 IP(`10.0.1.10:2222`) WARN은 benign으로 분리
- 호스트 경로(`127.0.0.1:2222`) 응답을 필수 체크로 승격

### A-6. P1 - 설치 시점 계정 강제 동기화
- `install-all.sh`는 Jenkins readiness 이후 `post-install.sh`를 자동 실행합니다.
- `post-install.sh`는 공통 관리자 비밀번호 기준으로 Nexus/Jenkins 계정을 설치 시점에 강제 동기화합니다.
- 운영 중 주기적 비밀번호 재설정은 수행하지 않으며, 설치/재설치 시점에만 적용합니다.
...
### install-all.sh가 "GitLab 준비 대기" 에서 오래 걸림
**원인**: 첫 부팅 시 DB 마이그레이션과 초기 설정에 시간이 오래 걸립니다. 최신 로직은
`/-/health` 대신 `/users/sign_in` 기준으로 준비 완료를 확인하므로, 이전보다 느리게 보일 수 있어도
재설치 없이 기다리는 것이 맞습니다.
...
`200 OK`가 반환되면 Jenkins 설치 단계로 진행됩니다.

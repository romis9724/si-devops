...
## 멱등성 표준 패턴 (devenv-security)

모든 자동 등록은 **조회 → (필요 시) 삭제/revoke 또는 update → 생성** 순서로 구현합니다. 동일 입력으로 재실행해도 중복 리소스가 쌓이지 않게 합니다.

| 대상 | 조회 | 정리(멱등) | 생성/갱신 |
|------|------|------------|-----------|
| SonarQube Global Analysis Token | `GET /api/user_tokens/search?login=...` | `POST /api/user_tokens/revoke` (동일 이름) | `POST /api/user_tokens/generate` |
| SonarQube Webhook | `GET /api/webhooks/list` | `POST /api/webhooks/delete` (key) | `POST /api/webhooks/create` |
| Jenkins SonarInstallation | `descriptor.getInstallations()` | 동일 `name` 제외 후 배열 재구성 | `new SonarInstallation(...)` + `setInstallations` |
| Jenkins Credential (sonar-token 등) | Credentials API / JCasC | 기존 id `delete` 또는 update | `create` (동일 id) |
| Jenkins POST (scriptText 등) | — | — | **`scripts/lib/jenkins.sh`로 crumb 강제**; 실패 시 `[SEC-E601]` |

구현 참고: `scripts/bootstrap-sonar.sh`, `scripts/jenkins-configure.sh`, `references/jenkins-sonar-installation.groovy`.
...
## 0-1. pre-app/post-app 프로필 오판별

### 증상
- `devenv-app`을 아직 설치하지 않았는데 PHASE 7 병합 안내가 표시됨
- 반대로 `devenv-app` 설치 후인데 병합 단계가 자동 건너뜀

### 원인
- `preset.json`의 `app` 섹션과 실제 샘플 앱 디렉토리 상태가 불일치
- 실행 중 재탐지 로직이 중복되어 프로필이 덮어쓰기됨

### 해결
1. PHASE 1에서 프로필을 1회만 확정하고(`pre-app` 또는 `post-app`) 이후 단계에서 재사용
2. 아래 항목을 동시에 점검
...
# 실제 샘플 앱 디렉토리 확인
test -d sample-apps/backend && echo "backend exists"
test -d sample-apps/frontend && echo "frontend exists"
```

3. 기준
- `app` 섹션 없음 + 샘플 앱 디렉토리 없음: `pre-app`
- `app` 섹션 있음 또는 샘플 앱 디렉토리 존재: `post-app`
